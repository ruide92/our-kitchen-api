...
### TASK-OWNER-PHONE-VISUAL-CORRECTION-11A (2026-09-05)

First-round owner phone visual acceptance: **VISUAL GATE = FAIL** (real device screenshots confirmed issues).

**Issues fixed in 11A:**

1. **Homepage session race**: onLoad cached empty v1_active_family_id before App bootstrap; onShow never re-read. Fixed: onShow is async, calls getApp().ensureSessionReady(), re-reads active_family_id from session state every time.
2. **Menu mini-cart / Shopping complete-bar covered by Custom TabBar**: Established Global Bottom Dock Contract in app.wxss (.tab-page-dock: position:fixed, bottom = calc(100rpx + safe-area + 16rpx)). Menu mini-cart and Shopping complete-bar use tab-page-dock class.
3. **Fridge canonical name lost**: Backend fridgeRow returns name, category_code, canonical_code. Frontend _enrichItem uses item.name first. listFridge SELECT includes i.category_code.
4. **Quantity display .000**: formatQuantity uses parseFloat().toString() to strip trailing zeros.
5. **Weekly Empty State styling**: Menu state-empty has flex column layout.
6. **vConsole**: WeChat client debug mode; user closes via top-right "..." -> 关闭调试.

**11A regressions fixed in 11B:**
- PROJECT_STATE.md encoding/line-break damage restored from 5764ddd, re-appended with Node fs utf8
- menu.wxss / shopping.wxss mojibake and unterminated comment restored from 5764ddd, re-applied with Node fs utf8
- WXSS comment sanity audit added (scripts/wxss-sanity-audit.js)
- Menu/Shopping scroll content bottom clearance added (tab-page-scroll-spacer)
- Shopping diners_count uses mealSummary.diners_count
- Fridge HTTP contract integration test added (real PostgreSQL)

**Tests:**
- Frontend payload/behavior: 25/25 PASS
- WXSS sanity audit: PASS
- Full npm run test:gate: 0 fail
- DevTools CLI preview: 0 project error

**VISUAL GATE: FAIL** (waiting for second-round owner phone verification)

**Next:** Owner phone second-round visual acceptance (4 checkpoints: homepage family name, menu mini-cart + scroll clearance, fridge real name + clean quantity + category, bottom sheet not covered; plus vConsole gone)
